...
export default function Carousel({
  ...
  const getSlideX = (i: number) => -(slideWidth + GAP) * i
  ...
  const dragConstraints = !loop ? { left: -trackWidth + slideWidth, right: 0 } : undefined
          ...
          {items.map((_, i) => (
            <div
              ...
              className={`h-4 w-4 rounded-full cursor-pointer transition-all duration-300 ${
                currentIndex === i ? "bg-gray-500" : "bg-white"
              }`}
            />
          ))}
        </div>
